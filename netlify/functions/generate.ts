    ...
    const framingRequest = fetcher("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          "Each of the first three framing sections receives the same approved non-project evidence pool. Decide which facts best support that section's purpose and the visitor's selected topics. Legacy narrative-role metadata is editorial guidance only and does not limit which facts may be used. You may use any subset of the supplied facts, including all of them when appropriate. Avoid unnecessary repetition across sections, but reuse a fact when it materially supports more than one section. Return the IDs of the facts materially used in the lead and detail in summary_evidence_fact_ids and detail_evidence_fact_ids.",
