...
import { approvedBenFactIds, assembleApprovedBenFactsNarrative } from "../src/shared/approved-benfacts";
...
describe("headline, lead, and progressive depth", () => {
  it.each(configurations.map((selection) => [selection]))("keeps the editorial hierarchy bounded for topics %j", (selection) => {
    const narrative = assembleApprovedBenFactsNarrative(selection);
  ...
  it("accepts the concise title style, including supported acronyms and curly apostrophes", () => {
    const narrative = assembleApprovedBenFactsNarrative([]);
    ...
    const result = applyAiFraming(framing, narrative, approvedBenFactIds);
