...
The executable evaluator prompt in `scripts/evaluation/evaluator.mjs` contains separate 1 through 5 anchors for every criterion and for overall quality. In that rubric, 3 is competent professional execution, 4 requires an identifiable strength beyond competence, and 5 requires an observable exceptional feature with little meaningful room for improvement. A criterion rationale that identifies a meaningful improvement cannot support 5, and overall 5 requires multiple genuine criterion-level 5 ratings whose strengths reinforce one another. This document summarizes that behavior; it is not read into evaluator requests.
